            ...
            for (Battleground::BattlegroundPlayerMap::const_iterator itr = bGround->GetPlayers().begin(); itr != bGround->GetPlayers().end(); ++itr)
                ...
                if (firstTeamID > 0 && secondTeamID > 0 && secondTeamMember)
                {
                    ...
                    if (firstTeam && secondTeam)
                    {
						...
						handler->PSendSysMessage("Use .Spec Leave to Leave Arena.");
                    }
                }
        }
...
public:
    ...
    {
        pPlayer->RemovePet(NULL, PET_SAVE_NOT_IN_SLOT, true);
        pPlayer->RemoveAurasByType(SPELL_AURA_MOUNTED);
        pPlayer->UnsummonPetTemporaryIfAny();
		pPlayer->ADD_GOSSIP_ITEM(1, "Spectate Rated 1v1", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_5V5_GAMES);
        pPlayer->ADD_GOSSIP_ITEM(1, "Spectate Rated 2v2", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_2V2_GAMES);
        pPlayer->ADD_GOSSIP_ITEM(1, "Spectate Rated 3v3", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_3V3_GAMES);
        pPlayer->ADD_GOSSIP_ITEM_EXTENDED(1, "Spectate Specific Player", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_SPECIFIC, "", 0, true);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        if (action >= NPC_SPECTATOR_ACTION_5V5_GAMES && action < NPC_SPECTATOR_ACTION_2V2_GAMES)
        {
            ...
            ShowPage(player, action - NPC_SPECTATOR_ACTION_5V5_GAMES, 1);
        ...
        else if (action >= NPC_SPECTATOR_ACTION_2V2_GAMES && action < NPC_SPECTATOR_ACTION_3V3_GAMES)
        {
            ...
            ShowPage(player, action - NPC_SPECTATOR_ACTION_2V2_GAMES, 2);
            player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
        }
        else if (action >= NPC_SPECTATOR_ACTION_3V3_GAMES && action < NPC_SPECTATOR_ACTION_SELECTED_PLAYER)
        {
            ...
            ShowPage(player, action - NPC_SPECTATOR_ACTION_3V3_GAMES, 3);
            player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
        }
        else
        {
